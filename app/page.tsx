...
export default function Home() {
  ...
  const [selectedCategory, setSelectedCategory] = useState("All"); // 🟢 เพิ่ม State หมวดหมู่
  ...
  // รายชื่อแบรนด์ที่มีในระบบ
  const brands = ["All", ...new Set(phones.map((p: any) => p.brand))];
  
  // 🟢 รายชื่อหมวดหมู่ที่ต้องการแสดง
  const categories = ["All", "Phone", "Case", "AirPods", "Apple Watch"];

  // ระบบ Filter: กรองตามชื่อ, แบรนด์ และ หมวดหมู่
  let filteredPhones = phones.filter((phone: any) =>
    (phone.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
    phone.brand.toLowerCase().includes(searchTerm.toLowerCase())) &&
    (selectedBrand === "All" || phone.brand === selectedBrand) &&
    (selectedCategory === "All" || phone.category === selectedCategory) // 🟢 กรองหมวดหมู่
  );
  ...
  return (
    <main className="min-h-screen bg-[#F2F2F7] font-sans text-[#1C1C1E]">
            ...
            {user ? (
              <div className="flex items-center space-x-3 bg-gray-100 p-1.5 pl-4 rounded-xl border border-gray-200">
                ...
                <div className="flex items-center gap-2">
                    <Link href="/orders">
                      <button className="text-[10px] font-black text-[#007AFF] uppercase hover:underline px-2 tracking-tighter">My Orders</button>
                    </Link>
                    {user.role === 'admin' && (
                      <Link href="/admin">
                        <button className="bg-black text-white px-3 py-1.5 rounded-lg text-[10px] font-black uppercase">Admin</button>
                      </Link>
                    )}
                    <button onClick={handleLogout} className="text-[10px] text-red-600 font-black border-l border-gray-300 pl-3 uppercase">Logout</button>
                </div>
              </div>
            ) : (
              <Link href="/login" className="text-xs font-black text-[#007AFF] border-2 border-[#007AFF] px-5 py-2 rounded-xl hover:bg-[#007AFF] hover:text-white transition-all uppercase tracking-widest">Login</Link>
            )}
            <NavbarCart />
          </div>
        </div>
      ...
      {!searchTerm && selectedBrand === "All" && selectedCategory === "All" && (
        <section className="bg-[#1C1C1E] text-white py-20 text-center relative overflow-hidden">
          ...
          <div className="max-w-4xl mx-auto px-6 relative z-10">
            <h2 className="text-5xl md:text-7xl font-black mb-6 tracking-tighter uppercase leading-none">UPGRADE YOUR <br/><span className="text-[#007AFF]">LIFESTYLE</span></h2>
            <p className="text-sm md:text-base text-gray-400 font-black uppercase tracking-[0.4em] opacity-80 mb-10">Premium Devices & Accessories</p>
      ...
      <section className="max-w-7xl mx-auto px-4 mt-12 space-y-6">
        {/* 🟢 ส่วนเลือกหมวดหมู่ (Category Tabs) */}
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 bg-white p-6 rounded-[2.5rem] border-2 border-gray-200 shadow-sm">
          <div className="flex flex-wrap gap-2">
            <span className="text-[10px] font-black text-gray-400 uppercase flex items-center mr-2">Brand:</span>
            {brands.map((brand) => (
              <button
                key={brand}
      ...
      <section className="max-w-7xl mx-auto px-4 py-16">
        {filteredPhones.length === 0 ? (
          <div className="py-32 text-center bg-white rounded-[3rem] border-2 border-dashed border-gray-200">
             ...
             <p className="text-lg font-black text-gray-300 uppercase tracking-widest">No items found in this category</p>
             <button onClick={() => {setSearchTerm(""); setSelectedBrand("All"); setSelectedCategory("All");}} className="mt-6 bg-[#007AFF] text-white px-8 py-3 rounded-xl font-black uppercase text-xs">Reset All</button>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
            {filteredPhones.map((phone: any) => (
              <div key={phone._id} className="bg-white rounded-[2.5rem] border-2 border-gray-200 p-8 flex flex-col hover:border-[#007AFF] hover:shadow-2xl hover:shadow-blue-50 transition-all duration-500 group relative">
                
                {/* Category Badge */}
                <div className="absolute top-6 left-6 bg-gray-100 text-[8px] font-black px-2 py-1 rounded text-gray-400 uppercase tracking-widest">{phone.category}</div>
                ...
                <div className="w-full h-64 bg-white mb-10 flex items-center justify-center rounded-2xl overflow-hidden p-4">
